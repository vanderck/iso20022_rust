...
use crate::messages::pacs00700113 as m;
...
impl Message for Document {
    async fn consume(
        ...
        let body = &doc.fi_to_fi_pmt_rvsl;
        let msg_type = "pacs.007.001.13";
        ...
        let txs = batch::extract_pacs007_txs(body);
        ...
        for etx in txs {
            ...
            let event = etx.into_event(SupportedMessage::Pacs007, JourneyEventKind::FiReversalRequested);
            H::process_event(scope.crud(), msg_type, &msg_id, event).await.map_err(|e| e.to_string())?;
        }
        let refs = first_refs.unwrap_or_default();
        let flow = FlowCoordinator::execute(scope.crud(), &ctx.db_pool, ctx.engine_client.clone(), &ctx.http_client, &ctx.config, SupportedMessage::Pacs007, &msg_id, &refs,
    ...
    async fn generate(&self, ctx: &crate::ProcessingContext) -> Result<String, String> {
        let now = chrono::Utc::now(); let msg_id = H::next_msg_id(&mut ctx.uidgen_client.clone(), "PACS007").await.map_err(|e| e.to_string())?;
        let body = m::FIToFIPaymentReversalV13 { grp_hdr: m::GroupHeader127 { msg_id: msg_id.clone(), cre_dt_tm: now, nb_of_txs: "1".into(), ..Default::default() }, ..Default::default() };
        let doc_xml = quick_xml::se::to_string(&m::Document::new(body)).map_err(|e| format!("{e}"))?;
        self.wrap(&doc_xml, "pacs.007.001.13", &msg_id, &ctx.config.own_bic, "RECEIVERBIC")
    }
}
