...
use crate::messages::camt05300113 as m;
...
/// Extract an IBAN or proprietary account id from an `AccountIdentification4Choice`.
...
/// `Stmt` blocks of a camt.053 into reconciliation-ready entries.
///
/// Previously this handler never looked past the message envelope: it
/// archived the raw XML and logged a single generic audit event keyed on the
/// statement's own message id (which can never resolve to a payment
/// journey), so bank-statement reconciliation never actually ran despite the
/// `Reconciliation` module existing for exactly this purpose.
pub fn extract_entries(body: &m::BankToCustomerStatementV13) -> Vec<StatementEntry> {
    let mut out = Vec::new();
    for stmt in &body.stmt {
        let acct_id = account_id(stmt.acct.id.as_ref());
...
impl Message for Document {
    async fn consume(
        ...
        let msg_type = "camt.053.001.13";
        let msg_id = doc.bk_to_cstmr_stmt.grp_hdr.msg_id.clone();
        ...
        let entries = extract_entries(&doc.bk_to_cstmr_stmt);
    ...
    async fn generate(&self, ctx: &crate::ProcessingContext) -> Result<String, String> {
        let now = chrono::Utc::now(); let msg_id = H::next_msg_id(&mut ctx.uidgen_client.clone(), "CAMT053").await.map_err(|e| e.to_string())?;
        let body = m::BankToCustomerStatementV13 { grp_hdr: m::GroupHeader116 { msg_id: msg_id.clone(), cre_dt_tm: now, ..Default::default() }, ..Default::default() };
        let doc_xml = quick_xml::se::to_string(&m::Document::new(body)).map_err(|e| format!("{e}"))?;
        self.wrap(&doc_xml, "camt.053.001.13", &msg_id, &ctx.config.own_bic, "RECEIVERBIC")
    }
}
...
mod tests {
    ...
    #[test]
    fn extract_entries_empty_statement_yields_no_rows() {
        let body = m::BankToCustomerStatementV13 { grp_hdr: m::GroupHeader116 { msg_id: "MSG1".into(), ..Default::default() }, ..Default::default() };
        assert!(extract_entries(&body).is_empty());
    }

    #[test]
    fn extract_entries_without_tx_details_yields_one_entry_level_row() {
        let body = m::BankToCustomerStatementV13 {
            grp_hdr: m::GroupHeader116 { msg_id: "MSG2".into(), ..Default::default() },
            stmt: vec![m::AccountStatement14 {
