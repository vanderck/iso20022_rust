...
use crate::messages::camt05400113 as m;
...
/// `Ntfctn` blocks of a camt.054 into reconciliation-ready entries. See
/// `handlers::camt053::extract_entries` for the same shape on end-of-day
/// statements; this handler previously had the identical gap (a generic,
/// self-referential audit event that never matched a real journey).
pub fn extract_entries(body: &m::BankToCustomerDebitCreditNotificationV13) -> Vec<StatementEntry> {
    let mut out = Vec::new();
    for ntfctn in &body.ntfctn {
        let acct_id = account_id(ntfctn.acct.id.as_ref());
...
impl Message for Document {
    async fn consume(
        ...
        let msg_type = "camt.054.001.13";
        let msg_id = doc.bk_to_cstmr_dbt_cdt_ntfctn.grp_hdr.msg_id.clone();
        ...
        let entries = extract_entries(&doc.bk_to_cstmr_dbt_cdt_ntfctn);
    ...
    async fn generate(&self, ctx: &crate::ProcessingContext) -> Result<String, String> {
        let now = chrono::Utc::now(); let msg_id = H::next_msg_id(&mut ctx.uidgen_client.clone(), "CAMT054").await.map_err(|e| e.to_string())?;
        let body = m::BankToCustomerDebitCreditNotificationV13 { grp_hdr: m::GroupHeader116 { msg_id: msg_id.clone(), cre_dt_tm: now, ..Default::default() }, ..Default::default() };
        let doc_xml = quick_xml::se::to_string(&m::Document::new(body)).map_err(|e| format!("{e}"))?;
        self.wrap(&doc_xml, "camt.054.001.13", &msg_id, &ctx.config.own_bic, "RECEIVERBIC")
    }
}
...
mod tests {
    ...
    #[test]
    fn extract_entries_empty_notification_yields_no_rows() {
        let body =
            m::BankToCustomerDebitCreditNotificationV13 { grp_hdr: m::GroupHeader116 { msg_id: "MSG1".into(), ..Default::default() }, ..Default::default() };
        assert!(extract_entries(&body).is_empty());
    }

    #[test]
    fn extract_entries_without_tx_details_yields_one_entry_level_row() {
        let body = m::BankToCustomerDebitCreditNotificationV13 {
            grp_hdr: m::GroupHeader116 { msg_id: "MSG2".into(), ..Default::default() },
            ntfctn: vec![m::AccountNotification24 {
