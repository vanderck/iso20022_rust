...
use crate::messages::camt02700110 as m;
...
impl Message for Document {
    async fn consume(
        ...
        let body = &doc.clm_non_rct;
        let msg_type = "camt.027.001.10";
        ...
        H::open_investigation(scope.crud(), jid.as_deref(), &refs, msg_type, None,
            &format!("Claim non receipt: assgnmt_id={}", msg_id)).await.map_err(|e| e.to_string())?;

        let event = JourneyEvent { source_message: SupportedMessage::Camt027,
            kind: JourneyEventKind::ClaimNonReceipt, refs: refs.clone(),
            iso_status: None, reason_code: None,
            description: format!("Claim non receipt: assgnmt_id={}", msg_id) };
        H::process_event(scope.crud(), msg_type, &msg_id, event).await.map_err(|e| e.to_string())?;

        let flow = FlowCoordinator::execute(scope.crud(), &ctx.db_pool, ctx.engine_client.clone(), &ctx.http_client, &ctx.config, SupportedMessage::Camt027, &msg_id, &refs,
    ...
    async fn generate(&self, ctx: &crate::ProcessingContext) -> Result<String, String> {
        ...
        let id = H::next_msg_id(&mut ctx.uidgen_client.clone(), "CAMT027").await.map_err(|e| e.to_string())?;
        let body = m::ClaimNonReceiptV10 { assgnmt: m::CaseAssignment6 { id: id.clone(), cre_dt_tm: now, ..Default::default() }, ..Default::default() };
        let doc_xml = quick_xml::se::to_string(&m::Document::new(body)).map_err(|e| format!("{e}"))?;
        self.wrap(&doc_xml, "camt.027.001.10", &id, &ctx.config.own_bic, "RECEIVERBIC")
    }
}
